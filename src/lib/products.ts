...
export function buildApiUrl(
	...
	const params = new URLSearchParams({
		...
		...(filters.unitPriceGt && { unit_price__gt: filters.unitPriceGt.toString() }),
		...(filters.unitPriceLt && { unit_price__lt: filters.unitPriceLt.toString() }),
...
export async function fetchInitialData() {
	try {
		const [productsResponse, collectionsResponse, featuresResponse] = await Promise.all([
